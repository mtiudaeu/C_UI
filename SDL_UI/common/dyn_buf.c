 ...
 SCOPE(allocator_cbs.free(tmp))
 {
  ...
  for(;i < size; i++) {
   ...
   while(i_child != 0) {
    ...
    //Not Tested : casting to (char*)
    void* child = (char*)data + (size_of_one*i_child);
    void* parent = (char*)data + (size_of_one*i_parent);
...
void _heapify(struct dyn_buf_info dyn_buf_info, void* data, int (*comparator)(void* lhs, void* rhs))
{
 ...
 SCOPE(allocator_cbs.free(tmp))
 {
  ...
  while(1)
  {
   ...
   if(i_child2 < size) {
    void* child1 = (char*)data + size_of_one*i_child1;
    void* child2 = (char*)data + size_of_one*i_child2;
    if(comparator(child1, child2) < 0) {
     i_max_child = i_child2;
    }
   }
   
   void* node = (char*)data + size_of_one*i_node;
   void* max_child = (char*)data + size_of_one*i_max_child;
...
void _dyn_buf_sort(struct dyn_buf_info dyn_buf_info, void* data)
{
 ...
 SCOPE(allocator_cbs.free(tmp))
 {
  for(int i = ((int)size)-1; i>0; i--) {
   void* first = data;
   void* last = (char*)data + i*size_of_one;
